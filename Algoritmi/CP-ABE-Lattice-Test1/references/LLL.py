...
def lovasz():
    global k
    '''Checks the Lovasz condition for a basis. Either swaps adjacent basis vectors and recomputes Gram-Scmidt or 
    increments the working index. '''
    ...
    if la.norm(orthobasis[k]) ** 2 >= np.dot(c, la.norm(orthobasis[k - 1] ** 2)):  # Check the Lovasz condition.
        ...
        basis[[k, k - 1]] = basis[[k - 1,
                                   k]]  # If the condition is not met, swap the working vector and the immediately preceding basis vector.
...
def main():
    while True:
        ...
        if x in ['Y', 'N', 'y', 'n']:
            ...
            input("Would you like to see the steps? Press [Y/N] and Enter. ")
    if x in ['Y', 'y']:
        gram_schmidt()
        steps = 0
